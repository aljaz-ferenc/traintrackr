...
export default function useDeleteNutrition() {
	...
	return useMutation({
		...
		mutationFn: ({nutritionId}:{nutritionId: Nutrition['_id'], date: Date}) =>
			toast.promise(
				createRequest({
        ...
        onMutate: async ({nutritionId, date}) => {
            await queryClient.cancelQueries({queryKey: ['nutrition-get', {date}]})
            const previousNutritions = queryClient.getQueryData(['nutrition-get', {date}])
            queryClient.setQueryData(['nutrition-get', {date}], (old: NutritionResponse) => {
                const deletedNutrition = old.nutritions.find(n => n._id === nutritionId)

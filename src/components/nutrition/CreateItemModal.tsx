...
export default function CreateItemModal({
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            if (editMode && !!defaultItem) {
                await updateFoodItem({...values, createdBy: userId as string, _id: defaultItem?._id})
            } else {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <VisuallyHidden>
                <DialogTitle>{editMode ? 'Update item' : 'Create New Item'}</DialogTitle>
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                        ...
                        <CardTitle className="flex gap-2 items-center">
                            Portions
                            <AppTooltip
                                content="Add different portion sizes to make tracking easier. For example, you can add a 'slice' or 'tablespoon' portion for your food item and specify how many grams the portion weighs!"/>
                        </CardTitle>
                        ...
                        <hr/>
                        <Button className='cursor-pointer' type="submit">{editMode ? 'Update item' : 'Submit'}</Button>
                    </form>
                </Form>
            </DialogContent>
        </Dialog>

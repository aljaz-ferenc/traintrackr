...
export default function CreateItemModal({isOpen, setIsOpen}: AddItemModalProps) {
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        await createNutrition({amount: values.amount * Number(values.portion.split('/')[0]), createdBy: userId, date: new Date(), item: getSelectedFoodItem()})
        setIsOpen(false)
        form.reset()
    }
                                                ...
                                                <FormItem>
                                                    ...
                                                    >
                                                        ...
                                                        <SelectContent>
                                                            ...
                                                            {foodItems && getSelectedFoodItem()?.portions?.map(portion => (
                                                                <SelectItem key={portion._id} value={`${portion.grams}/${portion._id}`}>{portion.name}</SelectItem>
                                                            ))}
                                                        </SelectContent>
                                                    </Select>
                                                    <FormMessage/>
